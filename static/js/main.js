(function ($) {
	...
	if ($('.setting-wrap').length) {
		...
		$elizaDropdown2.on('click', '.setting-active', function (e) {
			...
			if (!$this.parent().hasClass('show')) {
				...
				$this.siblings('.setting-content').removeClass('show').slideUp().parent().removeClass(
					'show');
			}
		});
		...
		$body2.on('click', function (e) {
			var $target = e.target;
			if (!$($target).is('.setting-wrap') && !$($target).parents().is('.setting-wrap') &&
	...
	$('[data-countdown]').each(function () {
		...
		$this.countdown(finalDate, function (event) {
			$this.html(event.strftime(
				'<span class="cdown day">%-D <p>Day</p></span> <span class="cdown hour">%-H <p>Hour</p></span> <span class="cdown minute">%M <p>Minute</p></span class="cdown sec"> <span>%S <p>Second</p></span>'
			));
		});
	});
	...
	$offCanvasNav.on('click', 'li a, li .menu-expand', function (e) {
		...
		if (($this.parent().attr('class').match(
				/\b(menu-item-has-children|has-children|has-sub-menu)\b/)) && ($this.attr('href') === '#' ||
